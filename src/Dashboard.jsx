...
export default function Dashboard({ onLogout }) {
  ...
  const [svcRoles, setSvcRoles] = useState([]); // selected role names
  ...
  async function addRole(e) {
    ...
    if (res.ok) {
      ...
      alert(d.error || "Failed to add role");
    }
  }
  ...
  async function addService(e) {
    ...
    if (!svcName || !svcBangla || svcRoles.length === 0) {
      alert("Fill name, Bangla name, and pick at least one role.");
      return;
    }
    const res = await apiFetch("/api/services", {
      ...
      body: JSON.stringify({
        name: svcName,
        nameBangla: svcBangla,
        helperRoles: svcRoles,
      }),
    });
    if (res.ok) {
      setSvcName("");
    ...
    <div className="dash">
      ...
      <div className="grid">
        ...
        <section className="panel">
          <h2>Helper Roles</h2>
          <p className="hint">
            The kinds of helpers in the system (Nurse, Doctor, Staff...).
          </p>
          ...
          <ul className="list">
            {roles.map((r) => (
              <li key={r._id} className="item">
                <span>
                  <strong>{r.nameBangla}</strong>{" "}
                  <span className="muted">({r.name})</span>
                </span>
                <button className="del" onClick={() => deleteRole(r._id)}>
                  ✕
                </button>
              </li>
            ))}
        ...
        <section className="panel">
          <h2>Services</h2>
          <p className="hint">
            What a patient can request. Choose which roles fulfill each service.
          </p>
          ...
          <form className="col" onSubmit={addService}>
            ...
            <input
              className="input"
              placeholder="সেবার নাম (বাংলা)"
              value={svcBangla}
              onChange={(e) => setSvcBangla(e.target.value)}
            />
            ...
            <div className="rolepick">
              ...
              {roles.map((r) => (
                <label key={r._id} className="chip">
                  <input
                    type="checkbox"
                    checked={svcRoles.includes(r.name)}
                    onChange={() => toggleSvcRole(r.name)}
                  />
                  {r.nameBangla}
                </label>
              ))}
              {roles.length === 0 && (
                <span className="muted">Add a role first.</span>
              )}
            </div>
